...
B<List> simple implementation for each case

=item * 

B<List> immutable data - the old version still exists, if you need it

=item * 

B<List> no having to shunt the right hand side of the list around in memory

=item * 

B<List> no having to reallocate and move the entire structure as it grows and
shrinks

=item * 

B<Array> Very fast for updating single elements in place

=item * 

B<Array> No having to copy the left-hand side of the array each time

=item * 

B<Array> The result will still be an array - this may be important if you will
...
from simply declaring the accessor as C<'ro'> (except the fear of shooting
ourselves in the feet later!)  We could alternatively retain the readonly
accessor, but set the value by direct access to the underlying hash, or
by calling methods on Moose's I<meta-object protocol>.

All of these would be hacks, I<but> they would be well encapsulated hacks,
i.e. only during the list construction, and we'd be entirely pure afterwards.
...
The solution in Perl is often to add a C<DESTROY> method (or, for Moose
objects, C<DEMOLISH>) that deletes the onward reference.  Again, this would
require some hacking to delete the reference behind the scenes, but would be
acceptable as it's encapsulated as part of object destruction.

But let's make C<Moose> work to earn its keep!  It actually comes with I<weak
reference> support by default.  This makes Perl's reference counting discount
the extra backward references when they're no longer required, avoiding the
problem.  We'd simply nead to tweak our declaration to:

#TODO test this, do I need to weaken 'next' too?

        has prev => ( isa=>'Any', is=>'ro', weak_ref=>1 );
