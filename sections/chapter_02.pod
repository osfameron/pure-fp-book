...
leak may eventually become a problem.  The solution is to add a C<DESTROY>
(or, for Moose objects, C<DEMOLISH>) method that deletes the onward reference.
Again, this would require some hacking to delete the reference behind the scenes,
but would be acceptable as it's encapsulated as part of object destruction.
