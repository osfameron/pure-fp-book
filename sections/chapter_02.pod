...
=item * convert the function into a tail-recursive one, by extracting out the stack

=item * convert the function into an iterative one, that manages its own stack

=back

The best general tutorial to these methods using a dynamic programming language
may well be Mark Jason Dominus's I<Higher Order Perl>, now freely available online
at http://hop.perl.plover.com/

We'll just briefly look at an example of each technique.

=head4 converting the function into a tail-recursive one
...
=head4 converting the function into an iterative one
...
values in a List until you get a single value.  An example could finding the
