...
But first, we'll look at an operation where arrays are less convenient:
...
Let's look at how we'd do that in a list:

    LIST
    0     1     2     3     4
    [a.]->[b.]->[c.]->[e.]->()

We need to re-point the link from C<c> to a new list with value C<d> that
points to the remainder of the list C<e>.
...
Note how similar this is to the C<updateAt> method!  We copy the head of the
list up until the point we want to insert, just as before.  The only difference
is that instead of linking the list to the original position's I<tail>, we link
to the whole list.
